...
| 3 — HRRR post-processing | Calibrated distribution from HRRR ensemble, with marine layer regime | NOAA NOMADS, GOES-18, KNKX/KVBG soundings | ⏳ (ingestion + regime calibration + METAR detector ✅) |
| 4 — Intraday nowcast | Real-time updates conditioning on observed temperature trajectory | METAR feed | ⏳ (max-so-far truncation ✅) |
| 5 — Strike pricing | Convert distribution → P(payout) per strike → mispricing vs. market | Layer 4 + Kalshi quotes | ⏳ (pricing + mispricing + Kelly sizing ✅) |
